...
def classify_file():
    ...
    for (top, left, bottom, right, rgb) in faces:
        if rgb is not None:
            ...
            crop_face = im.crop((left, top, right, bottom)).resize((FLAGS.image_size, FLAGS.image_size))
            ...
            draw_rectangle(dr, left, top, right, bottom, "red", 10)
...
def draw_rectangle(draw, left, top, right, bottom, color, border):
    for i in range(border):
        cor = (left, top, right, bottom)
        draw.rectangle(cor, outline=color)
    ...
    try:
        ...
        if (len(faces) > 0):
            for (x, y, w, h) in faces:
                ...
                face_im = cv2.resize(image[top:bottom, left:right], (FLAGS.image_size, FLAGS.image_size),
                                     cv2.INTER_CUBIC)
